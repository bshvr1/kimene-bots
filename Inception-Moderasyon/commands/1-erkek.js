...
class Erkek extends Command {
    ...
    if(!member.roles.cache.has(this.client.config.roles.manRoles[0])) {
        ...
        isimler.findOne({user: member.id}, async(err,res) => {
            if(!res) {
            let arr = []
            arr.push({isim: member.displayName, state: this.client.config.roles.manRoles, yetkili: message.author.id})
            ...
            res.isimler.push({isim: member.displayName, state: this.client.config.roles.manRoles, yetkili: message.author.id})
            res.save().catch(e => console.log(e))
          }
          })
    }
