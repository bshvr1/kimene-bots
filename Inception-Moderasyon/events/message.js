  ...
  async run(message) {
   ...
   // await this.client.spamMessage(message)
    ...
    for (const içindeki of prefikslerim) {
      if (message.content.startsWith(içindeki)) kimene = içindeki;
    }
    ...
    if (!cmd) {
      let res = await commandDBS.findOne({
        cmdName: message.content.split(" ")[0].slice(kimene.length)
      })
      if (!res) return
      if (res.allowedRoles.some(x => message.member.roles.cache.has(x)) == false && !res.allowedUsers.includes(message.author.id)) return
    ...
    if (!this.client.config.member.guildOwners.includes(message.author.id)) {
      ...
      if (blockArr.length == 9) {
        message.channel.send(`${message.author}` + "```⛔ Komut kullanımını kötüye kullandığın için engellendi. Açtırmak için ( Kimene#1000 / Kimene#0001 ) kişisine ulaşman gerekiyor.```")
        this.client.channels.cache.get(this.client.config.channels.commandBlockLog).send(`**${message.author.tag}** - ${message.author} (\`${message.author.id}\`) komut engeli yedi. | Komut kullanım özeti:\n\`\`\`${blockArr.map(x => x.içerik).join("\n")}\nKullandığı komutlar: ${blockArr.map(x => x.komut).join(",")}\nKullandığı kanallar: ${blockArr.map(x => x.kanal).join(",")}\`\`\``)
